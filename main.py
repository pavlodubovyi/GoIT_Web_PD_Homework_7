...
def get_student_join():
    students = conf.db_connection.session.query(Student).join(Student.teachers).all()
    for s in students:
...
def get_students():
    ...
    students = conf.db_connection.session.query(Student).options(subqueryload(Student.teachers)).limit(5).offset(0).all()
    for s in students:
        columns = ["id", "fullname", "teachers"]
        r = [dict(
            zip(columns, (s.id, s.fullname, [(t.id, t.fullname) for t in s.teachers])))]
...
def get_teachers():
    teachers = conf.db_connection.session.query(Teacher).options(joinedload(Teacher.students)).all()
    for t in teachers:
...
def get_teachers_outerjoin():
    ...
    teachers = conf.db_connection.session.query(Teacher).join(Teacher.students).all()
    for t in teachers:
        columns = ["id", "fullname", "students"]
        r = [dict(
            zip(columns, (t.id, t.fullname, [(s.id, s.fullname) for s in t.students])))]
...
def get_teachers_by_data():
    teachers = conf.db_connection.session.query(Teacher).options(joinedload(Teacher.students, innerjoin=True)) \
...
def get_students_with_contacts():
    students = conf.db_connection.session.query(Student).join(Contact).all()
    for s in students:
...
def get_info():
    students = (conf.db_connection.session.query(Student.id, Student.fullname, Teacher.fullname.label("teacher_fullname"),
...
def update_student(s_id, teachers: list[Teacher]):
    student = conf.db_connection.session.query(Student).filter_by(id=s_id).first()
...
def remove_student(s_id):
    student = conf.db_connection.session.query(Student).filter_by(id=s_id).first()
    conf.db_connection.session.delete(student)
    conf.db_connection.session.commit()
